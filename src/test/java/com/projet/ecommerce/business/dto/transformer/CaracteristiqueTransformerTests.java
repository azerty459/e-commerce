...
public class CaracteristiqueTransformerTests {
	...
	static {
		...
		PRODUIT_DTO.getPhotos().add(PHOTO_DTO1);
    ...
    public void listeDtoToEntity() {
        ...
        assertDataCaracteristique(caracteristiqueDTOList.get(1), caracteristiqueList.get(1));
    }
	...
	private void assertDataTypeCaracteristique(CaracteristiqueDTO caracteristiqueDto, Caracteristique caracteristique) {
		Assert.assertEquals(caracteristiqueDto.getTypeCaracteristiqueDto().getId(),
				caracteristique.getCaracteristiquePk().getTypeCaracteristique().getIdTypeCaracteristique());
		Assert.assertEquals(caracteristiqueDto.getTypeCaracteristiqueDto().getNomType(),
				caracteristique.getCaracteristiquePk().getTypeCaracteristique().getNomTypeCaracteristique());
	}

	private void assertDataProduit(ProduitDTO produitDto, Produit produit) {
		Assert.assertNotNull(produitDto);
		Assert.assertNotNull(produit);
		Assert.assertEquals(produitDto.getCategories().get(0).getNom(),
				produit.getCategories().get(0).getNomCategorie());
