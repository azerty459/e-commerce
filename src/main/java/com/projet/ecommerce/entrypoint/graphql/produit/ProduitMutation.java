...
public class ProduitMutation {
    ...
    public TypeRuntimeWiring produitWiring() {
        ...
        builder.dataFetcher("addProduit", (DataFetchingEnvironment environment) -> {
                    ...
                    return produitBusiness.add(environment.getArgument("ref"), environment.getArgument("nom"),
                    		environment.getArgument("description"), (float) prixHT, environment.getArgument("nouvelleCat"),
                    			environment.getArgument("caracteristiques")); 
                }
        );
